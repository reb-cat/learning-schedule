...
const KhalilDashboard = () => {
  ...
  const baseSchedule = getScheduleForStudentAndDay("Khalil", currentDay);
  ...
  const loadLastSyncTime = () => {
    const lastSync = localStorage.getItem('lastSync_Khalil');
    if (lastSync) {
  ...
  const syncWithCanvas = async () => {
    setSyncing(true);
    try {
      const assignments = await canvasService.getAssignmentsForStudent('Khalil');
      ...
      const now = new Date();
      localStorage.setItem('lastSync_Khalil', now.toISOString());
      setLastSynced(now);
  ...
  const toggleAssignmentComplete = (blockIndex: number) => {
    const assignment = assignmentScheduler.getAssignmentForBlock('Khalil', currentDay, blockIndex);
    if (assignment) {
      const newCompleted = !assignment.isCompleted;
      assignmentScheduler.markAssignmentCompleted('Khalil', currentDay, blockIndex, newCompleted);

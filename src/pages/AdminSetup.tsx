...
const AdminSetup = () => {
  ...
  const {
    ...
    refetch: refetchAbigail
  } = useAssignments('Abigail');
  const {
    assignments: khalilAssignments,
    loading: khalilLoading,
    refetch: refetchKhalil
  } = useAssignments('Khalil');

  // Fetch administrative notifications for badge count
  const {
    notifications
  } = useAdministrativeNotifications();
        ...
        <Tabs defaultValue="students" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="admin-tasks" className="flex items-center gap-2">
              ...
              Admin Tasks
              {pendingTasksCount > 0 && <Badge variant="destructive" className="ml-1 h-5 px-1.5 text-xs">
                  {pendingTasksCount}
                </Badge>}
            </TabsTrigger>
          ...
          <TabsContent value="students" className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Abigail Section */}
              <StudentSection studentName="Abigail" assignments={filterParentAssignments(abigailAssignments)} loading={abigailLoading} onAssignmentAdded={handleAssignmentAdded} />

              {/* Khalil Section */}
              <StudentSection studentName="Khalil" assignments={filterParentAssignments(khalilAssignments)} loading={khalilLoading} onAssignmentAdded={handleAssignmentAdded} />
            </div>
